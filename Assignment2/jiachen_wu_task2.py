...
threshold, support = int(sys.argv[1]), int(sys.argv[2])
...
start = time.time()
sc = SparkContext(master='local[*]', appName='inf553_hw2_task2')
min_partition_num = max(support//8, 1)
...
baskets = reviews.map(lambda x: [str(x.split(',')[0]), str(x.split(',')[1])]) \
    .groupByKey().mapValues(list).values().filter(lambda x: len(x)>threshold).map(set).persist()
